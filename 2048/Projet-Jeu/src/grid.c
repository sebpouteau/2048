...
static void move(grid g, int i, int j, int addi, int addj){
  ...
  for(int cpt = 0; cpt < GRID_SIDE; cpt++){
	...
	// On fusionne la case actuel avec la case compteur (la case actuel récupère la valeur de la case compteur)
	fusion(g, tmpi, tmpj, cpti, cptj);

      else if(get_tile(g, cpti, cptj) == get_tile(g, tmpi, tmpj)){
	// On fusionne la case actuel avec la case compteur 
	fusion(g, tmpi, tmpj, cpti, cptj);
	set_grid_score(g, get_tile(g, tmpi, tmpj));
	// On déplace la case actuel et la case compteur dans le sens défini
	incrementation(&tmpi, &tmpj, addi, addj);
	incrementation(&cpti, &cptj, addi, addj);
      }
      // On change la case actuel sur la même ligne/colonne suivant le sens défini
      else
	incrementation(&tmpi, &tmpj, addi, addj);
    }
...
/**
 * \brief test sur l'ensemble de la grille si le deplacement dans la direction voulue est faisable (en fonction des paramètres) 
 * \param g grid visée
