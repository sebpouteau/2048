...
static int nombre_case_vide(grid g){
...
static dir meilleur_direction(grid g){
  ...
  /*
  int nb_case_vide = nombre_case_vide(g);
  if(nb_case_vide < 5)
    nombre = 1;
  if(nb_case_vide < 3)
    nombre = PROFONDEUR + 1;*/
  ...
  for (int i = UP; i<=RIGHT ; i++){
	...
	for(int x = 0; x < GRID_SIDE; x++)
	  if(get_tile(g_copy,x,y) == 0){
	    ...
	    // Rappelle la fonction et multiplie le resultat par 9  car 10% de chance d'avoir un 4
...
long int note_grid (grid g){
  ...
  if(max > 256){
    cpt += pow(get_tile(g,0,0), 4);
    cpt += pow(get_tile(g,1,0), 4);
  }
  ...
  /* 	the_tile =(long int)get_tile(g,i,j); */
  /*     else */
  /* 	the_tile2=(long int)get_tile(g,i+1,j); */
  /*     if(the_tile == 0) */
  /* 	continue; */
  /*     else{ */
  /* 	if(prev == the_tile) */
  ...
  /*     merges += 1 + compt; */
  /* } */
  /* cpt += (long int)(merges*1243); //merges_weight == 700; */
  return cpt;
}
